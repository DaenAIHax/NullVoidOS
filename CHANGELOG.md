...
container-centric su Fedora Atomic) viene rimosso e **rimpiazzato**
dall'agent-primary OS, che diventa l'unica direzione del repo. Cancellati
`recipes/`, `modules/`, `files/`, `docs/` (INSTALLATION/System), il
`.github/workflows/build.yml` legacy, `cosign.pub` e il dev-shell flake di
root. Tutto il contenuto di `bootstrap/` è stato promosso alla root del repo
via `git mv` (history dei file preservata come rename); i riferimenti di path
*vivi* nei doc/sorgenti sono passati da `bootstrap/system/…` a `system/…` (le
voci storiche del CHANGELOG restano invariate come registro). `CLAUDE.md`
riscritto: rimossa la regola "due direzioni coesistono". `CODEOWNERS`
aggiornato a `@DaenAIHax`. Nuovo workflow CI `selfhost.yml` che certifica
l'Atto 1 (punto fisso self-host) sui runner GitHub-hosted; l'Atto 2 resta
locale perché richiede `/dev/kvm`. Gate Atto 1 ri-eseguito dalla root: PASS
(self0.c == self1.c == self2.c, 125280 byte). Lavoro su branch `going-public`,
da portare su `main` protetto via PR.
